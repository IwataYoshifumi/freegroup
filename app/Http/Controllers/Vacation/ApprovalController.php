...
class ApprovalController extends Controller
{
    ...
    public function monthly( Request $request ) {
        ...
        if( isset( $request->find )) {
            ...
            $find['show'] = [ '休暇種別' ,'ステータス' ];
            if( auth('user')->user()->browsing() == "全社" ) { array_push( $find['show'], '部署名'); }
        ...
        $dates = CommonController::getMonthlyCalendarDates( $base_date );
        ...
        // dd( $vacations );
        $vacation_ids = CommonController::get_array_dates_vacations_id( $vacations );
        
        // dd( $vacations, $vacation_ids );
        
        BackButton::setHere( $request );
        return View( 'vacation.approval.monthly' )->with( 'find', $find )
                                                  ->with( 'dates', $dates )
