...
<div class="clearfix">
    ...
    <div class="row">
        <div id="user_ids_form" class="font-weight-bold col-12">
            ...
            @foreach( $users as $c ) 
                <div class='col user_id' id='user_id_{{ $c->id }}' data-user_id='{{ $c->id }}'>
                    ...
                    <input type=hidden name='{{ $name }}[]' value={{ $c->id }}>{{ $c->name }}
                </div>
            @endforeach
        </div>
        ...
        <div class="col-7 btn-group">
            {{ Form::select( 'dept_id', Dept::getArrayforSelect(), old( 'dept_id' ),   [ 'class' => 'form-control', 'id' => 'dept_select', ] ) }}
            {{ Form::text( 'user_name', old( 'user_name' ), [ 'class' => 'form-control', 'id' => 'search_users', 'placeholder' => '名前' ] ) }}
        </div>
...
<script language='JavaScript'>
    ...
    function user_id_click( id, name ) {
        ...
        try {
            ...
            tag    += "     <input type=hidden name='{{ $name }}[]' value=" + id +">"+ name;
            tag    += "</div>";
            form.append( tag );
            // console.log( id );
        } catch( e ) {
