...
$max  = config( 'vacation.config.hourly_paidleave.max' );
$min  = config( 'vacation.config.hourly_paidleave.min' );

$hour =[];
$step = 60*8 * Vacation::get_houry_paid_leave_unit();

$start_time = Carbon::createFromTime( $min, 0, 0 );
$end_time   = Carbon::createFromTime( $max, 0, 0 );
// dump( $step, Vacation::get_houry_paid_leave_unit() );
// dump( $start_time, $end_time );
for( $i = $start_time; $i <= $end_time; $i->addMinutes( $step ) ) {
    $time = $i->format('H:i');
    $hour[$time] = $time;
}
//for( $i = $min; $i <= $max; $i++ ) {
        ...
        <div class="col-md-10">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('vacation.application.store_hourly' ) }}">
                        @csrf
                    ...
                    <div class="border border-dark rounded mt-3">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-7">
                                <input id="date" type="date" class="form-control @error('data') is-invalid @enderror" 
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-7">
                                @php
                                    $array = Vacation::getArrayForSelectOfHoulyPaidleaveApplicationForm( $user );
                                @endphp
                        ...
                        <div class="form-group row">
                            <label for="type" class="col-4 col-form-label text-md-right ml-2">休暇日</label>
                            <div class="col-7">
                        ...
                        <div class="form-group row">
                            <label for="type" class="col-4 col-form-label text-md-right ml-2">休暇時間</label>
                            @php
                                $max  = config( 'vacation.config.hourly_paidleave.max' );
                                $min  = config( 'vacation.config.hourly_paidleave.min' );
                                $step = config( 'vacation.config.hourly_paidleave.step' );
                                //dump( config( 'vacation.config.hourly_paidleave' ));
                            @endphp
                            <div class="col-7">
                                <!--
                                {{ Form::time( 'start_time', old( 'start_time' ), [ 'max' => $max, 'min' => $min, 'step' => $step, 'required', 'class' => 'form-control' ] ) }}
                                {{ Form::time( 'end_time', old( 'end_time' ), [ 'max' => $max, 'min' => $min, 'step' => $step, 'required', 'class' => 'form-control' ] ) }}
                                 -->
                                {{ Form::select( 'start_time', $hour, old( 'start_time' ), ['class' => 'form-control' ] ) }}
                                {{ Form::select( 'end_time',   $hour, old( 'end_time' ),   ['class' => 'form-control' ] ) }}                                
