    ...
    <div class="border border-dark rounded mt-3">
        ...
        <div class='p-1'>
            <div class="row">
                ...
                <div class="col-7 {{ optional( $statusClass )[$application->status] }}">{{ $application->status }}</div>
            </div>
            <div class="row">
        ...
        <table class='table table-border m-2'>
            ...
            @foreach( $approvals as $approval ) 
                <tr>
                    @auth( 'user' )
                        <td>
                            @if( auth('user')->id() == $approval->approver->id && $approval->status == "承認待ち" )
                                <a class="btn btn-sm btn-primary text-white" href="{{ route('vacation.approval.approve', [ 'approval' => $approval ] ) }}">承認</a>
                                <a class="btn btn-sm btn-danger  text-white" href="{{ route('vacation.approval.reject' , [ 'approval' => $approval ] ) }}">却下</a>
                            @endif
                        </td>
                    @endauth
