            ...
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        <div class="form-group row">
                            <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">タスクリスト名{!! $info !!}   {{-- htmlspecialchars OK --}}
                            </label>
                            <div class="col-md-6 m-1">
                                {{ $tasklist->name }}
                            </div>
                            
                            <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">タスクリスト表示名・色設定</label>
                            <div class="col-md-6 m-1" style='{{ $taskprop->style() }}'>
                                {{ $taskprop->name }}
                            </div>
                        
                            <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">タスクリスト公開種別{!! $info !!}</label>   {{-- htmlspecialchars OK --}}
                            <div class="col-md-6 m-1">
                            ...
                            @if( $tasklist->not_use )
                                ...
                                <div class="col-md-6 m-1"><span class="alert-danger p-2 text-dark">新規タスク　追加不可</span></div>
                            @endif

                            @if( $tasklist->disabled )
                                <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">【管理者設定】</label>
                                <div class="col-md-6 m-1"><span class="alert-danger p-2 text-dark">既存タスク　修正不可</span></div>
                            @endif
                            
                            <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">【個人設定】作成タスク変更権限（初期値）</label>
                            <div class="col-md-6 m-1">{{ $permissions[ $taskprop->default_permission ] }}</div>

                            @if( $taskprop->not_use and ! $tasklist->not_use )
                                <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">【個人設定】</label>
                                <div class="col-md-6 m-1"><span class="alert-warning p-2 text-dark">新規にタスクを作成しまし</span></div>
                            @endif

                            @if( $taskprop->hide )
                                <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">【個人設定】</label>
                                <div class="col-md-6 m-1"><span class="alert-warning p-2 text-dark">タスクを表示しない</span></div>
                            @endif
