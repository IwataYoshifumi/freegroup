        ...
        @elseif( $d->isSaturday() ) 
            ...
            <div class="row{{ $start_row }} col{{ $start_col }} span{{ $span }} cal3" style="pointer-events: none;">
                <div class="calendar_item order{{ $order }} {{ $schedule_class }} multi_schedule object_to_show_detail d-flex" style="{{ $style }}" {!! $data !!}> {{-- htmlspecialchars OK --}}
                    ...
                    <div class="flex-fill text-right time_item">
                        @if( ! $schedule->all_day )
            ...
            @if( $order < 6 )
                <div class="row{{ $start_row }} col{{ $start_col }} span1 cal3" style="pointer-events: none;">
                    <div class="calendar_item order{{ $order }} {{ $task_class }} {{ $complete }} single_schedule object_to_show_detail d-flex" {!! $data !!} style="{{ $style }}">
                        ...
                        <div class="text-right time_item">
                            {{ $task->p_time('daily') }}
                        </div>
                    </div>
                </div>
...
@if( ScreenSize::isMobile() )
    <script>
         ...
         *
         */
        $('.calendar_item').addClass( 'mb_calendar_item' );
        $('.mb_calendar_item').removeClass( 'calendar_item' );
        
        // $('.mb_calendar_item').css( 'font-size', 'xx-small' );
        $('.other_item').css( 'font-size', 'small' );
        $('.other_item').css( ' pointer-events', 'none' );
        $('.time_item').css( 'display', 'none');
        ...
        */
        ...
        $('.order5').css( 'top', '75px' );
        $('.order6').css( 'top', '90px' );
        $('.order7').css( 'top', '105px' );
        $('.order8').css( 'top', '130px' );
    </script>
@endif
