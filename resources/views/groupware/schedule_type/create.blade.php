        ...
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{ Form::open( [ 'route' => 'groupware.schedule.type.store', "enctype" => "multipart/form-data",  ] ) }}
                        @csrf
                        @method( 'POST' )
                        <div class="row">
                            <div class="col-3 m-1">スケジュール種別名</div>
                            {{ Form::text( 'input[name]', '', [ 'class' => 'col-7 form-control m-1' ] ) }}
                            
                            <div class="col-3 m-1">背景色</div>
                            {{ Form::color( 'input[color]', '#eeeeee', [ 'class' => 'col-1 form-control m-1', 'id' => 'color', 'onChange' => 'sample();' ] ) }}
                            <div class="col-2 p-1"><span id="sample1" class="m-2 p-1">色サンプル</span></div>
                            
                            <div class="col-12"></div>
                            
                            <div class="col-3 m-1">文字色</div>
                            {{ Form::color( 'input[text_color]', '#000000', [ 'class' => 'col-1 form-control m-1', 'id' => 'text-color', 'onChange' => 'sample()' ] ) }}
                            ...
                            {{ Form::text( 'input[google_calendar_id]', '', [ 'class' => 'col-7 form-control m-1' ] ) }}
                            
                            <div class="col-3 m-1">Google サービスアカウントID</div>
                            {{ Form::email( 'input[google_id]', '', [ 'class' => 'col-7 form-control m-1' ] ) }}
                            
                            <div class="col-3 m-1">Googleサービスアカウント秘密鍵</div>
                            {{ Form::file( 'google_private_key_file', [ 'class' => 'col-7 m-1' ] ) }}
                            
                            <div>
                                <button type='button' class="btn btn-primary m-3" onClick="this.form.submit()">新規スケージュール種別作成</button>
                                {{ BackButton::form() }}
                            </div>
