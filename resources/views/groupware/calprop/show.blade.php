        ...
        <div class="col-md-8">
            ...
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        @can( 'update', $calprop )
                            <a class="btn btn-outline-secondary text-dark" href="{{ $route_update_calprop }}">色設定・同期設定</a>
                        @endcan
                        
                        @can( 'view', $calendar )
                                <a class="btn btn-warning border-dark" href="{{ $route_show_calendar }}">カレンダー管理情報</a>
                        @endcan
                        
                        
                        
                        @if( $calprop->google_sync_on ) 
                            <a class="btn btn-warning border-dark" href="{{ $route_gsync }}">Googleカレンダー手動同期</a>
                        ...
                        <div class="form-group row">
                            ...
                            @if( $calendar->not_use or $calendar->disabled )
                                <label for="dept_id" class="col-md-4 col-form-label text-md-right m-1">管理者設定</label>
                                <div class="col-md-6 m-1">
                                    @if( $calendar->not_use )            
                                        予定追加不可（カレンダー管理者設定）<br>
                                    @endif
                                    @if( $calendar->disabled )            
                                        検索対象外・Google同期解除中（カレンダー管理者設定）
                                    @endif
                                </div>
                            @endif

                            @if( $calprop->not_use or $calprop->hide )
